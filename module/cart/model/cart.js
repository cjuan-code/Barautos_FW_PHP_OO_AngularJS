...
function assign_actions() {
    ...
    $(document).on('click', '.checkout', function() {
        ...
        if (token) {
            ...
            friendlyURL('?page=cart&op=factura').then(function(data) {
                ajaxPromise(data, 'POST', 'JSON', {user : user, total : total})
            });
            ...
            $('.product').each(function() {
                ...
                friendlyURL('?page=cart&op=line').then(function(data) {
                    ajaxPromise(data, 'POST' , 'JSON' ,{prod : product, qty : qty, price : linePrice, user : user, tyotal : total})
                });
...
function load_cars() {
    ...
    if (cars_mat) {
        ...
        friendlyURL('?page=cart&op=cars').then(function(data) {
            ...
            .then(function(data_cart) {
                ...
                for (row in data_cart) {
                    ...
                    $('<img></img>').attr('src','http://localhost/' + data_cart[row].img).appendTo('.img_'+cont);
